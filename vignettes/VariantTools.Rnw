...
p53 <- gmapR:::exonsOnTP53Genome("TP53") + 1e5
...
step).
<<tallyVariants>>=
raw.variants <- tallyVariants(bam, tally.param, mc.cores=1)
@ 
...
during exploratory analysis. The filters include a minimum read count
(2) check, minimum unique read position count (2) check, and Fisher Exact Test
on the per-strand counts vs. reference for strand bias (p-value
cutoff: 0.001). If there are at least three read position bins in the tallies,
at least one read must present the variant in an internal read position
bin. The intent is to ensure that we have sufficient data and that the
data are not due to strand-specific nor read position-specific artifacts.
...
head(softFilterMatrix(qa.variants))
@ 
