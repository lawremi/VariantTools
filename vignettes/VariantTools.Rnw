...
We can also manipulate the filters that call the variants that have
already passed the basic QA checks.
...
% We have already called variants for the metastatic H1993 sample. We
...
% control.raw <- tallyVariants(bams$H2073, tally.param)
% control.called.noqa <- callVariants(control.raw)
% control.called <- callVariants(qaVariants(control.raw))
% control.cov <- coverage(bams$H2073, drop.D.ranges = TRUE)
% filters <- SampleSpecificVariantFilters(control.raw, control.cov,
%                                         VariantCallingFilters())
...
% somatic <- callSampleSpecificVariants(tumor.bam, normal.bam, tally.param,
...
variants. This requirement is an attempt to ensure consistency. This
