TwoSampleFET <- function(gr, raw, cov, pval=0.001, freq=0.05){
  ...
  over2 <- which(paste(values(gr)$location,values(gr)$alt) %in% paste(values(raw)$location, values(raw)$alt)) 
  over <- which(paste(values(raw)$location,values(raw)$alt) %in% paste(values(gr)$location, values(gr)$alt))
